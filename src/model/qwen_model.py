...
def generate_output(config, dataset, processed_dataset, model, processor, trained_model: bool = False):
    ...
    for batch in tqdm(dataloader):
        ...
        if outputs:
            for output in outputs:
                ...
                processed_dataset[idx][f'{"trained" if trained_model else "base"}_model_output'] = output
                idx += 1
...
def get_model(config, trained_model: bool = False):
    processor = AutoProcessor.from_pretrained(config['base_model_path'], trust_remote_code=True, use_fast=True)

    model = AutoModelForImageTextToText.from_pretrained(config['base_model_path'], trust_remote_code=True)
